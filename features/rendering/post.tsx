...
import { getBlogHref, getDictionary, getLocalizedPath, getOtherLocale, getPostHref, type Locale } from "@/lib/i18n";
import type { Metadata } from "next";
import Link from "next/link";
import { notFound } from "next/navigation";

export async function getPostMetadata(locale: Locale, slug: string): Promise<Metadata> {
  const post = await getPostBySlug(slug, locale);
  const canonical = getPostHref(locale, slug);
  ...
  const translatedPost = await getPostByTranslationKey(post.translationKey, targetLocale);

  return {
    metadataBase: new URL(getBaseUrl()),
    alternates: {
      canonical,
      languages: translatedPost
        ? {
            [locale]: getLocalizedPath(locale, `/blog/${slug}`),
            [targetLocale]: getPostHref(targetLocale, translatedPost.slug),
          }
        : undefined,
    },
  };
...
export async function renderPostPage(locale: Locale, slug: string) {
